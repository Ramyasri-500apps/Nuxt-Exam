<template>
  <table class="min-w-full divide-y divide-gray-700">
    <thead>
      <tr>
        <th v-for="(field, index) in fields" :key="index" scope="col" class="py-3 5 pl-4 pr-3 text-left text-sm font-semibold sm:pl-0 group">
          {{ field }}
        </th>
      </tr>
    </thead>
                ...
                <div class="flex h-full flex-col divide-y divide-gray-200 bg-white shadow-xl">
                  <div class="flex min-h-0 flex-1 flex-col overflow-y-scroll py-6">
                    <div class="px-4 sm:px-6">
                      <div>
                        <DialogTitle class="text-base font-semibold leading-6 text-gray-900">Edit Student</DialogTitle>
                        <div>
                          <StudentEdit
                            :student="editStudent"
                            @edit="
...
<script setup lang="ts">
  ...
  const fields = ref(["Name", "Age", "Gender", "Date Of Birth", "class"]);
  ...
  const emitData = (student: any, index: any, value: any) => {
    if (value == "edit") {
      editStudent.value = JSON.parse(JSON.stringify(student));
      edit.value = true;
      return;
    }
    emit("emitData", { student: student, value: value, index: index });
  };
</script>
